...
*/
    return ((fMem[n] + u[n+nx]*coef[0] + u[n-nx]*coef[1] + u[n+1]*coef[2] + u[n-1]*coef[3]) / uDivisor);
}

double subsRow(int n, double *u, double uDivisor, double hx, double hy, int nx, double *coef) {
/*
f(x,y) =
...
*/
    return (uDivisor * u[n] - (u[n+nx]*coef[0] + u[n-nx]*coef[1] + u[n+1]*coef[2] + u[n-1]*coef[3]));
}
...
//void sor(double *x, double *r, double *timeSor, double *timeResNorm, double w, double uDivisor, double hx, double hy, int nx, int ny, int maxI) {
	int i, j, k;
	double sigma, now, fxy, res, maxRes = 0, tRes = 0, *coef; // maxRes is the biggest residue, tRes is total residue in this iteration.

    if((coef = malloc(4 * sizeof(double))) == NULL) {
	...
	for(k=0; k<maxI; ++k) {
	    ...
	    for(i=1; i<ny-1; ++i) { // Ignoring borders.
	        for(j=1; j<nx-1; ++j) { // Ignoring borders as well.
	            x[i*nx+j] = x[i*nx+j] + w * (calcU(i*nx+j,x,fMem,uDivisor,hx,hy,nx,coef) - x[i*nx+j]);
	        }
	    }
		...
		*timeSor += timestamp() - now; // Get iteration time.
	    ...
	    for(i=1; i<ny-1; ++i) { // Ignoring borders.
	        for(j=1; j<nx-1; ++j) { // Ignoring borders as well.
	            res = fMem[i*nx+j] - subsRow(i*nx+j,x,uDivisor,hx,hy,nx,coef);
				if(res > maxRes)
					maxRes = res;
